...
class ActorCritic(OnPolicy):
    def __init__(self, in_shape, num_actions):
        ...
        fc_size = 256
        self.fc = nn.Sequential(nn.Linear(self.feature_size(), fc_size), nn.ReLU())
    ...
    def feature_size(self):
        convoutput1 = self.calculate_conv_output(
            self.in_shape[1:3], self.out_channels, 3
        )
        convoutput2 = self.calculate_conv_output(
            convoutput1[1:3], self.out_channels, 3, 2
        )
        features = int(np.prod(convoutput2))
        return features

    def calculate_conv_output(
        self, img_dim, out_channels, kernel_size, stride=1, padding=0
    ):
...
class RolloutStorage(object):
    ...
    def compute_returns(self, next_value, gamma):
        ...
        for step in reversed(range(self.num_steps)):
            returns[step] = (
                returns[step + 1] * gamma * self.masks[step + 1] + self.rewards[step]
            )
        return returns[:-1]
